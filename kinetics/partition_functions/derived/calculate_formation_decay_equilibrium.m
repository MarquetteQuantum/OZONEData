function Kfds_m3 = calculate_formation_decay_equilibrium(states, temp_k, part_funcs_o2_per_m3, threshold_energies_j, ...
  optional)
...
% 1st column - channel 1, 2nd - channel 2 (not used for 666)
  arguments
    states
    temp_k
    part_funcs_o2_per_m3
    threshold_energies_j
    optional.numerator_only = false
  end
  ...
  for K_ind = 1:size(threshold_energies_j, 1)
    for sym_ind = 1:size(threshold_energies_j, 2)
      ...
      for ch = 1:size(threshold_energies_j, 3)
        ...
        if ~optional.numerator_only
          Kfds_m3(inds, ch) = part_funcs_o3_i / part_funcs_o2_per_m3(K_ind, sym_ind, ch);
        end
      end
    end
  end
